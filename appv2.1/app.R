...
peaks_graph_smooth <- function(df_tidy, df_tidy_smooth){
  peaks_graph_smooth<- ggplot2::ggplot() + 
    ...
    ggplot2::facet_wrap(~ as.numeric(strip)) +
    ggplot2::theme_light() +
...
titlePanel("CAA curvefit"),
  sidebarLayout(
    sidebarPanel(
      ...
      sliderInput("range", "Set control peak detection range", min = 40, max = 100, value = c(55,90))
    ),
    ...
    mainPanel(
      tabsetPanel(
        tabPanel("Raw and smoothed scans", 
                  fluidRow(
                    column(11, plotOutput("rawvssmooth")),
                    column(1, downloadButton("downloadrawvssmooth", "Download plot"))
                  )
        ),
        #tabPanel("control peaks", plotOutput("plotlistcontrol")),
        tabPanel("All peaks", 
                  plotOutput("plotlistall"),
                  downloadButton("downloadPeaks", "Download plot", height = "10%")
        ),
...
observeEvent(input$startAnalysis, {
  ...
  test_samples<-reactive({ 
    n<-as.character(
      if(input$radio == 1 & ncol(peaks()) == 10){
        NA
      }else{
        if(input$radio == 1){
          11:ncol(peaks())
        }else{
          if(input$radio == 2 & ncol(peaks()) == 20){
            NA
          }else{
            if(input$radio == 2){
              21:ncol(peaks())
            }else{
              if(input$radio == 3 & ncol(peaks()) == 30){
                NA
              }else{
                if(input$radio == 3){
                  31:ncol(peaks())
  ...
  output$rawvssmooth <- renderPlot(
    ...
    peaks_graph_smooth(df_tidy(), df_tidy_smooth_samples())}),
    width = 1300, height = function() 450+(12 * ncol(peaks())))
  
  
  #detect control peak
  peak_list_control <- reactive({peak_list_ctrl(peaks_smooth_bc(), input$range)})
  
observeEvent(input$range, {
  Sys.sleep(1.5)
  output$plotlistcontrol <- renderPlot(
    withProgress(message = "Plotting", value = 0,{
    plot_list_control(peaks_smooth_bc(), df_tidy_smooth(), peak_list_control())}), 
    width = 900, height = function() 450+(45 * ncol(peaks())))
})
  #detect test peak
  peaks_list <- reactive({test_peak_detect(peak_list_control(), peaks_smooth_bc())})
  output$plotlistall <- renderPlot(
    withProgress(message = "Plotting", value = 0,{
    plot_list_all(peaks_smooth_bc(), df_tidy_smooth(), peaks_list())}), 
    width = 900, height = function() 450+(45 * ncol(peaks())))
  
  #AUC calculations
  peaks_list_filled <- reactive({lapply(peaks_list(), fill_matrix)})
  peaks_df <- reactive({extract_peak_data(peaks_list_filled())})
  peaks_data <- reactive({merge_peak_data(df_tidy_smooth_samples(), peaks_df())})
  
  auc_T <- reactive({auc_calculation(x = peaks_smooth_bc(), 
                                     y = peaks_data(), 
                                     z = strip_line_short(), 
                                     peak = "T")
  })
  
  auc_C <- reactive({(auc_calculation(x = peaks_smooth_bc(), 
                                      y = peaks_data(), 
                                      z = strip_line_short(), 
                                      peak = "C"))
  })
  ...
  #Standard curve
  peaks_data_standard <- reactive({peaks_data_auc() %>% 
      tidyr::drop_na(caa) %>%
      dplyr::select(sample_name, caa, `T/C`)
  })
  
  peaks_data_standard_names <- reactive({(name_curves(peaks_data_standard()))})
  output$standardcurve <- renderPlot({rawplot(peaks_data_standard_names())})
  standard_df <- reactive({std_df(peaks_data_standard_names())})
  output$standardaverage <- renderPlot(averageplot(standard_df()), width = 900, height = 900)
  curvemodel <- reactive(model(standard_df(), peaks_data_standard_names()))
  output$plotfitcurve <- renderPlot(curvefitplot(curvemodel(), peaks_data_standard_names(), input$assay)) #still behaves weird
  
  cutoff <- reactive({cutoffcalc(curvemodel(), peaks_data_standard_names())})
  output$cutoffs <- renderText(c(paste0("Calcultated cutoff = ", round(cutoff()[1,1], digits = 2), "pg/ml")))
  ...
  results <- reactive({as.data.frame(resultfile(peaks_data_auc(), curvemodel(), peaks_data_standard_names(), value(), input$assay))})
  ...
  output$downloadrawvssmooth = downloadHandler(
    ...
    content = function(file) {
      ...
      width_inches <- 12  # Set the desired width in inches
      height_inches <- 8 # Set the desired height in inches
    ...
    withProgress(message = "Downloading", value = 0,{
      print(peaks_graph_smooth(df_tidy(), df_tidy_smooth_samples()))
      })
      dev.off()
    }
  )
  ...
  output$downloadPeaks = downloadHandler(
    ...
    content = function(file) {
      ...
      width_inches <- 12  # Set the desired width in inches
      height_inches <- 4+(0.4 * ncol(peaks())) # Set the desired height in inches
