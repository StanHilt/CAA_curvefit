...
ui <- fluidPage(
  ...
  # Application title
  titlePanel("Labrox wet CAA analysis v3 (Salus Algorithm)"),
  ...
  sidebarLayout(
    sidebarPanel(
      ...
      selectInput(
        ...
        choices = list("UCAAhT17 cutoff: 6pg/ml" = 1, "UCAAhT417 cutoff: 0.6pg/ml" = 2, "SCAA20 cutoff: 10pg/ml" = 3, "SCAA500 cutoff: 1pg/ml" = 4), 
        selected = 1
      ),
...
server <- function(input, output, session) {
  ...
  observeEvent(input$startAnalysis, {
    ...
    full_caa <- reactive({addcaa(peaks_data_auc(), curvemodel())})
